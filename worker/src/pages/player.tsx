...
interface SessionScore { sessionId: number; seq: number; label: string; total: number; status: string }
...
const PlayerPage: FC<PlayerPageProps> = ({ code, circleName, playerName, total, seasonRank, roundsPlayed, best, worst, sessionsPlayed, wins, sessionScores }) => {
  const avgPerSession = sessionsPlayed > 0 ? total / sessionsPlayed : 0;
  const avgPerRound = roundsPlayed > 0 ? total / roundsPlayed : 0;
  ...
  return (
    <Layout title={`${playerName} — ${circleName}`}>
      ...
      <div class="grid grid-cols-2 gap-3 mb-6">
        {[
          { label: 'Total Poin', value: total > 0 ? `+${total}` : total, color: total >= 0 ? 'text-good' : 'text-bad', icon: '🎯' },
          { label: 'Poin / Sesi', value: formatAvg(avgPerSession), color: avgPerSession >= 0 ? 'text-good' : 'text-bad', icon: '📈' },
          { label: 'Poin / Ronde', value: formatAvg(avgPerRound), color: avgPerRound >= 0 ? 'text-good' : 'text-bad', icon: '📊' },
            ...
            {sessionScores.map((s) => (
              ...
              >
                <div class="flex items-center gap-2">
                  <span class="text-xs font-mono font-bold text-muted w-8 text-center">#{s.seq}</span>
