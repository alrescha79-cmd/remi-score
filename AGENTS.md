...
- Zustand stores in `src/store/`, persisted to `expo-sqlite/kv-store`. **Persist rehydrates async** — do not initialize component state from a store value on first render; read the store directly.
